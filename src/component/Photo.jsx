      ...
      >
        {photos.map((item , index) => {
          if ( item.id < (currentPathname.includes("/contact") ? 100 : 4 ) ) {
            return (
          <div
          className="w-full h-[18rem] relative group max-sm:py-5 "
          key={item.id}
          >
            <div
            className={`w-full h-full filter group-hover:lg:brightness-50 ${isOpen ? "brightness-[0.2]" : ""} `}
            >
              <img 
            ...
            <button
            className="group-hover:lg:block hidden top-[7rem] right-[12rem] absolute
            max-xl:right-[7rem]
            "
            ...
            {/*  */}
            <div
            className={` absolute -top-[3rem] w-full h-full z-10 
              max-sm:hidden
              ${isOpen % 3 === 0 ?   "xl:-right-[56rem] lg:-right-[40rem]" :
                ( isOpen % 3 === 2 ? "xl:-right-[26rem] lg:-right-[20rem]" :
                                      "right-[2rem]" )}
            ...
            <button
            className={`absolute -top-[4rem] z-20
               ${isOpen % 3 === 0 ?   "xl:-right-[46rem] lg:-right-[33rem]" :
                ( isOpen % 3 === 2 ? "xl:-right-[15rem] lg:-right-[13rem]" :
                                      "xl:right-[13rem] lg:right-[9rem]" )}
            ${ ( isOpen === item.id ) ? "block" : "hidden"} `}
